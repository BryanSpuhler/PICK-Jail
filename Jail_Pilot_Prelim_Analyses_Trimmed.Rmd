...
Jail_clean %>% 
    ...
    dplyr::select(Age, Age_Groups, 
                  Gender,
                  Ethnicity, Ethnicity_Dich,
                  Education_collapsed, Education_3cat,
                  RelationshipEducation, RelEd_Dich, 
                  Income, Income10K,
                  Gender, Div_Dich,
                  FinancialWorry_cat,
...
Jail_clean_outcome_pre_corr <- Jail_clean %>% 
    ...
    dplyr::select(LoveSufficientMarriage_T1.n, LoveEnoughSustain_T1.n, FoolWalksAway_T1.n, 
                  PaceRelationshipSafe.n, SpotWarningSigns.n, WeighProsCons.n, 
                  KeepPartnerDoing_T1.n, ForbidPartnerTalking_T1.n, WouldNotStay_T1.n) %>% 
    
    dplyr::mutate(`LoveSuffMar` = LoveSufficientMarriage_T1.n) %>% 
    dplyr::mutate(`LoveSust` = LoveEnoughSustain_T1.n) %>% 
    dplyr::mutate(`FoolsWalk` = FoolWalksAway_T1.n) %>% 
    dplyr::mutate(`PaceSafe` = PaceRelationshipSafe.n) %>% 
    dplyr::mutate(`SpotWarn` =  SpotWarningSigns.n) %>% 
    dplyr::mutate(`WeighPros` = WeighProsCons.n) %>% 
    dplyr::mutate(`KeepDoing` = KeepPartnerDoing_T1.n) %>% 
    dplyr::mutate(`ForbidPart` = ForbidPartnerTalking_T1.n) %>% 
    dplyr::mutate(`WouldNotStay` = WouldNotStay_T1.n) %>% 
     
    dplyr::select(`LoveSuffMar`, `LoveSust`, `FoolsWalk`,
                  `PaceSafe`, `SpotWarn`, `WeighPros`, `KeepDoing`,
                  `ForbidPart`, `WouldNotStay`) %>% 
...
corrplot::corrplot(Jail_clean_outcome_pre_corr$r, 
         ...
         number.cex= 0.7,
         tl.cex = .7,
         title = "Pre-Program Item Correlations",
                             mar=c(0,0,1,0))
...
Jail_clean_outcome_post_corr <- Jail_clean %>% 
    dplyr::filter(in.analysis.sample3 == TRUE) %>%
    dplyr::select(LoveSufficientMarriage_T2.n, LoveEnoughSustain_T2.n, FoolWalksAway_T2.n, 
                  PaceRelationshipSafe_After.n, SpotWarningSigns_After.n, WeighProsCons_After.n, 
                  KeepPartnerDoing_T2.n, ForbidPartnerTalking_T2.n, WouldNotStay_T2.n) %>% 
    
    dplyr::mutate(`LoveSuffMar` = LoveSufficientMarriage_T2.n) %>% 
    dplyr::mutate(`LoveSust` = LoveEnoughSustain_T2.n) %>% 
    dplyr::mutate(`FoolsWalk` = FoolWalksAway_T2.n) %>% 
    dplyr::mutate(`PaceSafe` = PaceRelationshipSafe_After.n) %>% 
    dplyr::mutate(`SpotWarn` =  SpotWarningSigns_After.n) %>% 
    dplyr::mutate(`WeighPros` = WeighProsCons_After.n) %>% 
    dplyr::mutate(`KeepDoing` = KeepPartnerDoing_T2.n) %>% 
    dplyr::mutate(`ForbidPart` = ForbidPartnerTalking_T2.n) %>% 
    dplyr::mutate(`WouldNotStay` = WouldNotStay_T2.n) %>% 
...
#### Pre-Program to Post-Program Item-Level
...
Jail_clean_outcome_prepost_corr <- Jail_clean %>% 
    ...
    dplyr::mutate(`LoveSuffMarPre` = LoveSufficientMarriage_T1.n) %>% 
    dplyr::mutate(`LoveSustPre` = LoveEnoughSustain_T1.n) %>% 
    dplyr::mutate(`FoolsWalkPre` = FoolWalksAway_T1.n) %>% 
    dplyr::mutate(`PaceSafePre` = PaceRelationshipSafe.n) %>% 
    dplyr::mutate(`SpotWarnPre` =  SpotWarningSigns.n) %>% 
    dplyr::mutate(`WeighProsPre` = WeighProsCons.n) %>% 
    dplyr::mutate(`KeepDoingPre` = KeepPartnerDoing_T1.n) %>% 
    dplyr::mutate(`ForbidPartPre` = ForbidPartnerTalking_T1.n) %>% 
    dplyr::mutate(`WouldNotStayPre` = WouldNotStay_T1.n) %>%
    
    dplyr::mutate(`LoveSuffMarPost` = LoveSufficientMarriage_T2.n) %>% 
    dplyr::mutate(`LoveSustPost` = LoveEnoughSustain_T2.n) %>% 
    dplyr::mutate(`FoolsWalkPost` = FoolWalksAway_T2.n) %>% 
    dplyr::mutate(`PaceSafePost` = PaceRelationshipSafe_After.n) %>% 
    dplyr::mutate(`SpotWarnPost` =  SpotWarningSigns_After.n) %>% 
    dplyr::mutate(`WeighProsPost` = WeighProsCons_After.n) %>% 
    dplyr::mutate(`KeepDoingPost` = KeepPartnerDoing_T2.n) %>% 
    dplyr::mutate(`ForbidPartPost` = ForbidPartnerTalking_T2.n) %>% 
    dplyr::mutate(`WouldNotStayPost` = WouldNotStay_T2.n) %>% 

    
    dplyr::select(`LoveSuffMarPre`, `LoveSustPre`, `FoolsWalkPre`,
                  `PaceSafePre`, `SpotWarnPre`, 
                  `WeighProsPre`, `KeepDoingPre`, `ForbidPartPre`,
                  `WouldNotStayPre`,
                  
                  `LoveSuffMarPost`, `LoveSustPost`, `FoolsWalkPost`,
                  `PaceSafePost`, `SpotWarnPost`, 
                  `WeighProsPost`, `KeepDoingPost`, `ForbidPartPost`,
                  `WouldNotStayPost`) %>% 
    # cor(use = "pairwise.complete.obs")
    as.matrix() %>% 
    Hmisc::rcorr(type = "pearson")
...
corrplot::corrplot(Jail_clean_outcome_prepost_corr$r[1:9,9:18],
                   number.digits = 2,
         method="color", 
         col=col(200),  
         type="upper", 
         ...
         tl.col="black", tl.srt=45, #Text label color and rotation
         # Combine with significance
         p.mat = Jail_clean_outcome_prepost_corr$P[1:9,9:18], 
         sig.level = 0.05, 
         insig = "blank", 
         # hide correlation coefficient on the principal diagonal
         diag=FALSE, 
         number.cex= 0.6,
         tl.cex = .5,
         title = "Pre-Post Program Item Correlations",
                             mar=c(0,0,1.7,0))
...
Jail_clean_outcomes_corr <- Jail_clean %>% 
    dplyr::filter(in.analysis.sample3 == TRUE) %>%
    dplyr::select(BL_T1, BL_T2, VF_T1, VF_After, Control_T1, Control_T2, 
                  BL_Change, VF_Change, Control_Change) %>%
    
    dplyr::mutate(`BlindLove Pre` = BL_T1) %>% 
    dplyr::mutate(`VF Scale Pre` = VF_T1) %>% 
    dplyr::mutate(`Control Pre` = Control_T1) %>% 
    dplyr::mutate(`BlindLove Post` = BL_T2) %>% 
    dplyr::mutate(`VF Scale Post` = VF_After) %>% 
    dplyr::mutate(`Control Post` = Control_T2) %>%
    dplyr::mutate(`BlindLove Change` = BL_Change) %>% 
    dplyr::mutate(`VF Scale Change` = VF_Change) %>% 
    dplyr::mutate(`Control Change` = Control_Change) %>% 
    
    dplyr::select(`BlindLove Pre`, `VF Scale Pre`, 
                  `Control Pre`, `BlindLove Post`,
                  `VF Scale Post`, `Control Post`,
                  `BlindLove Change`, `VF Scale Change`,
                  `Control Change`) %>% 
